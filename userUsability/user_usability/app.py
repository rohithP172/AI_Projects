...
def user_friendly():
    if request.method == 'POST':
        session['first_impression'] = request.form.get('first_impression')
        session['navigation_easy'] = request.form.get('navigation_easy')
        session['colors_readability'] = request.form.get('colors_readability')
        session['overall_comfort'] = request.form.get('overall_comfort')
        session['visual_appeal'] = request.form.get('visual_appeal')
        session['emoji_feedback'] = request.form.get('emoji_feedback')
        session['mobile_tested'] = request.form.get('mobile_tested')
        session['user_comments'] = request.form.get('user_comments')
        return redirect(url_for('user_friendly_result'))
    return render_template('user_friendly.html')

@app.route('/user_friendly_result')
def user_friendly_result():
    return render_template('user_friendly_result.html', data=session)

# -------------------- COGNITIVE WALKTHROUGH (PURE) --------------------
